...
def create_darkroom_env(env_name, dataset_size, n_envs):
    ...
    train_envs = [
        DarkroomEnvVec(train_envs[i:i+n_envs]) for i in range(0, len(train_envs), n_envs)
    ]
    test_envs = [
        DarkroomEnvVec(test_envs[i:i+n_envs]) for i in range(0, len(test_envs), n_envs)
    ]
    eval_envs = [
        DarkroomEnvVec(eval_envs[i:i+n_envs]) for i in range(0, len(eval_envs), n_envs)
    ]
...
def create_keydoor_env(env_name, dataset_size, n_envs):
    ...
    locations = np.array([[(j, i) for i in range(dim)] for j in range(dim)]).reshape(-1, 2)
    ...
    train_test_split = int(.8 * len(key_door_pairs))
    ...
    train_envs = [KeyDoorEnv(dim, locations[key], locations[door], environment_horizon, markovian) for key, door in train_goals]
    eval_envs = [KeyDoorEnv(dim, locations[key], locations[door], environment_horizon, markovian) for key, door in eval_goals]
    test_envs = [KeyDoorEnv(dim, locations[key], locations[door], environment_horizon, markovian) for key, door in test_goals]

    train_envs = [
        KeyDoorVecEnv(train_envs[i:i+n_envs]) for i in range(0, len(train_envs), n_envs)
    ]
    test_envs = [
        KeyDoorVecEnv(test_envs[i:i+n_envs]) for i in range(0, len(test_envs), n_envs)
    ]
    eval_envs = [
        KeyDoorVecEnv(eval_envs[i:i+n_envs]) for i in range(0, len(eval_envs), n_envs)
    ]
    return train_envs, test_envs, eval_envs
...
def create_navigation_env(env_name, dataset_size, n_envs):
    ...
    goals = np.array([[radius * np.cos(angle), radius * np.sin(angle)] for angle in angles])
    np.random.RandomState(seed=0).shuffle(goals)
    train_test_split = int(.8 * len(goals))
    train_goals = goals[:train_test_split]
    test_goals = goals[train_test_split:]
    ...
    train_envs = [NavigationEnv(radius, goal, environment_horizon, dense_reward) for goal in train_goals]
    eval_envs = [NavigationEnv(radius, goal, environment_horizon, dense_reward) for goal in eval_goals]
    test_envs = [NavigationEnv(radius, goal, environment_horizon, dense_reward) for goal in test_goals]

    train_envs = [
        NavigationVecEnv(train_envs[i:i+n_envs]) for i in range(0, len(train_envs), n_envs)
    ]
    test_envs = [
        NavigationVecEnv(test_envs[i:i+n_envs]) for i in range(0, len(test_envs), n_envs)
    ]
    eval_envs = [
        NavigationVecEnv(eval_envs[i:i+n_envs]) for i in range(0, len(eval_envs), n_envs)
    ]
    return train_envs, test_envs, eval_envs
