...
def rollin_mdp(env, rollin_type, horizon):
    states = []
    actions = []
    next_states = []
    rewards = []
    ...
    for _ in range(horizon):
        ...
        if hasattr(env, "sample_flags"):  # for KeyDoorEnv
            have_keys, have_doors = env.sample_flags(state)
            next_state, reward, _, _ = env.transit(state, action, have_keys, have_doors)
        else:
    ...
    for env in tqdm.tqdm(envs):
        for j in range(n_hists):
            (
                ...
                context_next_states,
                context_rewards,
            ) = rollin_mdp(env, rollin_type=rollin_type, horizon=horizon)
            for k in range(n_samples):
                query_state = env.sample_state()
                if hasattr(env, "sample_flags"):
                    have_keys, _ = env.sample_flags(query_state)
                    optimal_action = env.opt_action(query_state, have_keys)
                else:
                    optimal_action = env.opt_action(query_state)

                for env_id in range(len(env._envs)):    
                    traj = {
                        'query_state': query_state[env_id],
                        'optimal_action': optimal_action[env_id],
                        'context_states': context_states[env_id],
                        'context_actions': context_actions[env_id],
                        'context_next_states': context_next_states[env_id],
                        'context_rewards': context_rewards[env_id],
...
def generate_darkroom_histories(envs, dim, horizon, **kwargs):
    # envs = [darkroom_env.DarkroomEnv(dim, goal, horizon) for goal in goals]
    # envs = [darkroom_env.DarkroomEnvVec(envs[i:i+n_envs]) for i in range(0, len(envs), n_envs)]
    trajs = generate_mdp_histories_from_envs(envs, horizon=horizon, **kwargs)
    return trajs
...
if __name__ == '__main__':
    ...
    if True:
        ...
        train_trajs = generate_darkroom_histories(train_envs, **config)
        test_trajs = generate_darkroom_histories(test_envs, **config)
        eval_trajs = generate_darkroom_histories(eval_envs, **config)
